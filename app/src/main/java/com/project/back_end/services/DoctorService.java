...
//    - Instruction: Ensure that the time slots are properly formatted and the available slots are correctly filtered.
    ...
    public List<String> getDoctorAvailability(long doctorId, LocalDate date){
        ...
        List<Appointment> appointments = appointmentRepository.findByDoctorIdAndAppointmentTimeBetween(doctorId,date.atStartOfDay(),date.atTime(23, 59));

        if(appointments.isEmpty()) return doctor.getAvailableTimes();

        return doctor.getAvailableTimes().stream().filter(time -> {
            for(Appointment appointment: appointments){
                LocalDateTime start= appointment.getAppointmentTime();
                String aux = ""+start.getHour()+":"+start.getMinute()+"-"+(start.getHour()+1)+":"+start.getMinute();

                if(time.equals(aux)) return false;
            }
    ...
    public int updateDoctor(Doctor doctor){
        try {
            ...
            if(!doctorDb.isPresent()) return -1;
    ...
    public int deleteDoctor(long doctorId){
        try {
           ...
           if(!docToDelete.isPresent()) return -1;
    ...
    public ResponseEntity<Map<String, Object>> validateDoctor(Login login){
        try {
            ...
            if(doctor == null) return ResponseEntity.badRequest().body(invalid);

            if(!login.getPassword().equals(doctor.getPassword())) return ResponseEntity.badRequest().body(invalid);
            ...
            System.out.println(e.getMessage());
            return ResponseEntity.badRequest().body(Map.of("error", true, "token", null, "message","An internal error ocurred. Try again later!"));
        }
    }
    ...
    public Map<String, Object> findDoctorsByNameSpecialtyandTime(String name, String specialty, String amPm){
        try {
            ...
            return Map.of("doctors", doctors.stream().filter(doc -> doc.getAvailableTimes().stream().anyMatch(time -> time.contains(amPm))).collect(Collectors.toList()));
